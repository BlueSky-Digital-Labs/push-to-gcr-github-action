...
#                Optional env values are: INPUT_IMAGE_TAG,INPUT_DOCKERFILE,INPUT_TARGET,INPUT_CONTEXT,INPUT_BUILD_ARGS,INPUT_NEW_TEST
...
if [ "$INPUT_PUSH_ONLY" = true ]; then
    ...
    echo "test example"
    echo "$INPUT_NEW_TEST" | python -m base64 -d
    ...
    if [ ! -z "$INPUT_NEW_TEST" ]; then
        ...
        BUILD_PARAMS="$BUILD_PARAMS --build-arg SSH_PRIV=${INPUT_NEW_TEST}"
    fi
